...
class MultilayerNN(nn.Module):
    ...
    def __init__(self, center_dim, offset_dim):
        ...
        self.final = nn.Parameter(torch.FloatTensor(center_dim, center_dim))
        # every time the initial is different
        nn.init.xavier_uniform(self.final)
        self.register_parameter("final", self.final)

    def forward(self, center_emb, offset, three=None):
        ...
        temp2 = F.relu(torch.matmul(offset, self.mats1_offset))
        temp3 = torch.cat([temp1, temp2], dim=2)

        combined = F.relu(torch.matmul(temp3, self.post_mats_offset))

        out_offset = F.relu(torch.matmul(combined, self.final))

        return out_offset

class CenterFineturn(nn.Module):
    def __init__(self, center_dim, offset_dim):
        super(CenterFineturn, self).__init__()
        self.center_dim = center_dim
        self.offset_dim = offset_dim

        self.pre_mats = nn.Parameter(torch.FloatTensor(center_dim + offset_dim, center_dim))
        nn.init.xavier_uniform(self.pre_mats)
        self.register_parameter("pre_mats", self.pre_mats)

        self.post_mats = nn.Parameter(torch.FloatTensor(center_dim, center_dim))
        nn.init.xavier_uniform(self.post_mats)
        self.register_parameter("post_mats", self.post_mats)

        self.mats1 = nn.Parameter(torch.FloatTensor(center_dim, center_dim))
        nn.init.xavier_uniform(self.mats1)
        self.register_parameter("mats1", self.mats1)

        self.mats2 = nn.Parameter(torch.FloatTensor(center_dim, center_dim))
        nn.init.xavier_uniform(self.mats2)
        self.register_parameter("mats2", self.mats2)

    def forward(self, center_emb, offset, three=None):

        temp1 = F.relu(torch.matmul(center_emb, self.mats1))
        temp2 = F.relu(torch.matmul(offset, self.mats2))

        temp3 = torch.cat([temp1, temp2], dim=2)
        combined = F.relu(torch.matmul(temp3, self.pre_mats))
        out_offset = (torch.matmul(combined, self.post_mats))
        return out_offset
...
class Query2box(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma,
                 ...
                 gamma2=0, bn='no', nat=1, activation='relu', center_fineturn=False, loss_shift=True,
                 cascade=True):
        ...
        # -------
        self.center_fineturn = center_fineturn
        self.loss_shift = loss_shift
        self.cascade = cascade
        self.center_trans = MultilayerNN(hidden_dim, hidden_dim)
        self.center_fineturn_neural = CenterFineturn(hidden_dim, hidden_dim)
        # -------
    ...
    def BoxTransE(self, head, relation, tail, mode, offset, head_offset, rel_len, qtype, shift_of_node):
        ...
        if qtype == 'chain-inter':
            ...
            for i in range(2):
                ...
                offset_temp = offsets[i][:, 0, :, :]
                offset_temp = self.center_trans(query_center_1, offset_temp)
                if self.cascade:
                    query_min_1 = query_min_1 - 0.5 * self.func(offset_temp) + relations[i][:, 0, :, :]
                    query_max_1 = query_max_1 + 0.5 * self.func(offset_temp) + relations[i][:, 0, :, :]
                else:
                    query_min_1 = query_center_1 - 0.5 * self.func(offset_temp)
                    query_max_1 = query_center_1 + 0.5 * self.func(offset_temp)

            query_center_2 = heads[1] + relations[2][:, 0, :, :]
            offset_temp = offsets[2][:, 0, :, :]
            offset_temp = self.center_trans(query_center_2, offset_temp)
            query_min_2 = query_center_2 - 0.5 * self.func(offset_temp)
            query_max_2 = query_center_2 + 0.5 * self.func(offset_temp)

            if self.euo:
                if self.cascade:
                    query_min_1 = query_min_1 - 0.5 * self.func(head_offsets[0])
                    query_min_2 = query_min_2 - 0.5 * self.func(head_offsets[1])
                    query_max_1 = query_max_1 + 0.5 * self.func(head_offsets[0])
                    query_max_2 = query_max_2 + 0.5 * self.func(head_offsets[1])
            ...
            shifts = torch.chunk(shift_of_node, 3, dim=0)
            ...
            if self.euo:
                if self.cascade:
                    offset_temp1 = self.center_trans(query_center_1, offsets[0][:, 0, :, :])
                    offset_temp2 = self.center_trans(query_center_2, offsets[1][:, 0, :, :])
                    ...
                    offset_temp1 = self.center_trans(query_center_1, offsets[0][:, 0, :, :])
                    offset_temp2 = self.center_trans(query_center_2, offsets[1][:, 0, :, :])
                    query_min_1 = query_center_1 - 0.5 * self.func(offset_temp1)
                    query_min_2 = query_center_2 - 0.5 * self.func(offset_temp2)
                    query_max_1 = query_center_1 + 0.5 * self.func(offset_temp1)
                    query_max_2 = query_center_2 + 0.5 * self.func(offset_temp2)
            else:
                offset_temp1 = self.center_trans(query_center_1, offsets[0][:, 0, :, :])
                offset_temp2 = self.center_trans(query_center_2, offsets[1][:, 0, :, :])
                query_min_1 = query_center_1 - 0.5 * self.func(offset_temp1)
            ...
            offset_temp = self.center_trans(new_query_center, offsets[2][:, 0, :, :])
            if self.cascade:
                new_query_min = new_query_center - 0.5 * self.func(new_offset) - 0.5 * self.func(offset_temp)
                new_query_max = new_query_center + 0.5 * self.func(new_offset) + 0.5 * self.func(offset_temp)
            else:
                new_query_min = new_query_center - 0.5 * self.func(offset_temp)
                new_query_max = new_query_center + 0.5 * self.func(offset_temp)
            ...
            if self.euo:
                if self.cascade:
                    offset_temp = head_offset
                    offset_temp = self.center_trans(query_center, offset_temp)
                    query_min = query_min - 0.5 * self.func(head_offset)
                    query_max = query_max + 0.5 * self.func(head_offset)

            if self.cascade:
                for rel in range(rel_len):
                    query_center = query_center + relation[:, rel, :, :]
                    offset_temp = offset[:, rel, :, :]
                    offset_temp = self.center_trans(query_center, offset_temp)
                    # hist higher
                    query_min = query_min - 0.5 * self.func(offset_temp) + relation[:, rel, :, :]
                    query_max = query_max + 0.5 * self.func(offset_temp) + relation[:, rel, :, :]
            else:
                for rel in range(rel_len):
                    query_center = query_center + relation[:, rel, :, :]
                    offset_temp = offset[:, rel, :, :]
                    offset_temp = self.center_trans(query_center, offset_temp)
                    query_min = query_center - 0.5 * self.func(offset_temp)
                    query_max = query_center + 0.5 * self.func(offset_temp)
