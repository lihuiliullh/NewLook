...
class Query2box(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma, 
        ...
        self.initial_node_embedding = nn.Parameter(torch.tensor(initial_node_embedding).float())
        self.initial_predicate_embedding = nn.Parameter(torch.tensor(initial_predicate_embedding).float())
        self.weight_parameter = nn.Parameter(torch.tensor([10]).float())
    ...
    def forward(self, sample, rel_len, qtype, mode='single'):
        if qtype == 'chain-inter':
            ...
            if mode == 'single':
                ...
                head_1_ = torch.index_select(self.initial_node_embedding, dim=0, index=sample[:, 0]).unsqueeze(1)
                head_2_ = torch.index_select(self.initial_node_embedding, dim=0, index=sample[:, 2]).unsqueeze(1)
                head_ = torch.cat([head_1_, head_2_], dim=0)
                ...
                if rel_len == 3:
                    ...
                    head_3_ = torch.index_select(self.initial_node_embedding, dim=0, index=sample[:, 4]).unsqueeze(1)
                    head_ = torch.cat([head_, head_3_], dim=0)
                ...
                tail_ = torch.index_select(self.initial_node_embedding, dim=0, index=sample[:,-1]).unsqueeze(1)


                if rel_len == 2:
                    tail = torch.cat([tail, tail], dim=0)
                    tail_ = torch.cat([tail_, tail_], dim=0)

                elif rel_len == 3:
                    tail = torch.cat([tail, tail, tail], dim=0)
                    tail_ = torch.cat([tail_, tail_, tail_], dim=0)
                ...
                # ---------------------------------------------------------------------
                relation_1_ = torch.index_select(self.initial_predicate_embedding, dim=0,
                                                 index=sample[:, 1]).unsqueeze(
                    1).unsqueeze(1)
                relation_2_ = torch.index_select(self.initial_predicate_embedding, dim=0,
                                                 index=sample[:, 3]).unsqueeze(
                    1).unsqueeze(1)
                relation_ = torch.cat([relation_1_, relation_2_], dim=0)
                if rel_len == 3:
                    relation_3_ = torch.index_select(self.initial_predicate_embedding, dim=0,
                                                     index=sample[:, 5]).unsqueeze(
                        1).unsqueeze(1)
                    relation_ = torch.cat([relation_, relation_3_], dim=0)
                # ---------------------------------------------------------------------------
                ...
                head_1_ = torch.index_select(self.initial_node_embedding, dim=0, index=head_part[:, 0]).unsqueeze(1)
                head_2_ = torch.index_select(self.initial_node_embedding, dim=0, index=head_part[:, 2]).unsqueeze(1)
                head_ = torch.cat([head_1_, head_2_], dim=0)
                ...
                if rel_len == 3:
                    ...
                    head_3_ = torch.index_select(self.initial_node_embedding, dim=0, index=head_part[:, 4]).unsqueeze(1)
                ...
                tail = torch.index_select(self.entity_embedding, dim=0, index=tail_part.view(-1)).view(batch_size, negative_sample_size, -1)
                tail_ = torch.index_select(self.initial_node_embedding, dim=0, index=tail_part.view(-1)).view(batch_size, negative_sample_size, -1)
                ...
                # ---------------------------------------------------------------------
                relation_1_ = torch.index_select(self.initial_predicate_embedding, dim=0, index=head_part[:, 1]).unsqueeze(
                    1).unsqueeze(1)
                relation_2_ = torch.index_select(self.initial_predicate_embedding, dim=0, index=head_part[:, 3]).unsqueeze(
                    1).unsqueeze(1)
                relation_ = torch.cat([relation_1_, relation_2_], dim=0)
                if rel_len == 3:
                    relation_3_ = torch.index_select(self.initial_predicate_embedding, dim=0, index=head_part[:, 5]).unsqueeze(
                        1).unsqueeze(1)
                    relation_ = torch.cat([relation_, relation_3_], dim=0)
                # ---------------------------------------------------------------------------
        ...
        if self.model_name in model_func:
            if qtype == '2-inter' or qtype == '3-inter' or qtype == '2-union' or qtype == '3-union':
                score, score_cen, offset_norm, score_cen_plus, _ = model_func[self.model_name](head, relation, tail, mode, offset, head_offset, 1, qtype, head_, relation_, tail_)
            else:
                score, score_cen, offset_norm, score_cen_plus, _ = model_func[self.model_name](head, relation, tail, mode, offset, head_offset, rel_len, qtype, head_, relation_, tail_)
        else:
            raise ValueError('model %s not supported' % self.model_name)
        
        return score, score_cen, offset_norm, score_cen_plus, None, None
    
    def BoxTransE(self, head, relation, tail, mode, offset, head_offset, rel_len, qtype, head_, relation_, tail_):
        ...
        if qtype == 'chain-inter':
            ...
            ######------------------------------------------------------------------------------
            my_query_center = head_

            for rel in range(rel_len__):
                # the final mean
                rel_m = relation_[:, rel, :, :]
                rel_m = rel_m.squeeze(1)
                my_query_center = torch.matmul(my_query_center, rel_m)

            rel_len = int(qtype.split('-')[0])
            assert rel_len > 1

            my_queries_center = torch.chunk(my_query_center, rel_len, dim=0)

            tails_ = torch.chunk(tail_, rel_len, dim=0)

            my_new_query_center = my_queries_center[0]
            for i in range(1, rel_len, 1):
                my_new_query_center = my_new_query_center + my_queries_center[i]

            my_new_query_center = my_new_query_center / rel_len
            dist = F.relu(tails_[0] - my_new_query_center)
            dist = torch.norm(dist, p=2, dim=2)
            # score_ is distance, the lower, the better
            score_ = dist

        score = self.gamma.item() - torch.norm(score_offset, p=1, dim=-1) - self.weight_parameter * score_
        score_center = self.gamma2.item() - torch.norm(score_center, p=1, dim=-1) - self.weight_parameter * score_
        score_center_plus = self.gamma.item() - torch.norm(score_offset, p=1, dim=-1) - self.cen * torch.norm(score_center_plus, p=1, dim=-1) - self.weight_parameter * score_
