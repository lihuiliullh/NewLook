...
class MultilayerNN(nn.Module):
    ...
    def __init__(self, center_dim, covariance_dim):
        super(MultilayerNN, self).__init__()
        self.center_dim = center_dim
        self.covariance_dim = covariance_dim

        assert center_dim == covariance_dim
        ...
        self.mats1_covariance = nn.Parameter(torch.FloatTensor(center_dim, expand_dim))
        nn.init.xavier_uniform(self.mats1_covariance)
        self.register_parameter("mats1_covariance", self.mats1_covariance)
        ...
        self.post_mats_covariance = nn.Parameter(torch.FloatTensor(expand_dim * 2, center_dim))
        # every time the initial is different
        nn.init.xavier_uniform(self.post_mats_covariance)
        self.register_parameter("post_mats_covariance", self.post_mats_covariance)

    def forward(self, center_emb, covariance_emb):
        ...
        temp2 = F.relu(torch.matmul(covariance_emb, self.mats1_covariance))

        temp3 = torch.cat([temp1, temp2], dim=2)

        out_center = torch.matmul(temp3, self.post_mats_center)
        out_covariance = torch.matmul(temp3, self.post_mats_covariance)
        return (out_center, out_covariance)
...
class Query2box(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma,
                 ...
                 node_one_hot_embedding=None, predicate_trans_matrix_embedding=None, use_center_trans=False):
        ...
        self.use_center_trans = use_center_trans

        self.cov_func = F.relu

        # -------
        if self.use_center_trans:
            self.center_trans_network = MultilayerNN(hidden_dim, hidden_dim)
        # -------
    ...
    def GaussianTrans(self, head_center, relation_center, tail_center, mode, head_covariance, relation_covariance,
        ...
        if qtype == 'chain-inter':
            ...
            query_covariance_1 = self.cov_func(relation_covariances[0][:, 0, :, :]) - self.cov_func(head_covariances[0])
            query_covariance_1 = self.cov_func(query_covariance_1)
            if self.use_center_trans:
                query_center_1, query_covariance_1 = self.center_trans_network(query_center_1, query_covariance_1)
                query_covariance_1 = self.cov_func(query_covariance_1)

            query_center_1 = query_center_1 + relation_centers[1][:, 0, :, :]
            query_covariance_1 = self.cov_func(relation_covariances[1][:, 0, :, :]) - self.cov_func(query_covariance_1)
            query_covariance_1 = self.cov_func(query_covariance_1)
            if self.use_center_trans:
                query_center_1, query_covariance_1 = self.center_trans_network(query_center_1, query_covariance_1)
                query_covariance_1 = self.cov_func(query_covariance_1)
            ...
            query_covariance_2 = self.cov_func(relation_covariances[2][:, 0, :, :]) - self.cov_func(head_covariances[1])
            query_covariance_2 = self.cov_func(query_covariance_2)
            if self.use_center_trans:
                query_center_2, query_covariance_2 = self.center_trans_network(query_center_2, query_covariance_2)
                query_covariance_2 = self.cov_func(query_covariance_2)
            ...
            new_covariance = self.cov_func(new_covariance)

            score_ = new_query_center - tail_center
            offset_score_ = new_covariance - self.cov_func(tail_covariance)
            ...
            new_query_convariance = self.cov_func(new_query_convariance)

            if self.use_center_trans:
                new_query_center, new_query_convariance = self.center_trans_network(new_query_center, new_query_convariance)

            score_ = new_query_center - tail_center
            offset_score_ = new_query_convariance - self.cov_func(tail_covariance)
            ...
            query_center_1 = query_center_1 + relation_centers[2][:, 0, :, :]
            query_covariance_1 = self.cov_func(relation_covariances[2][:, 0, :, :]) - query_covariance_1
            query_covariance_1 = self.cov_func(query_covariance_1)
            if self.use_center_trans:
                query_center_1, query_covariance_1 = self.center_trans_network(query_center_1, query_covariance_1)
                query_covariance_1 = self.cov_func(query_covariance_1)
            ...
            query_center_2 = head_centers[1] + relation_centers[1][:, 0, :, :]
            query_covariance_2 = self.cov_func(relation_covariances[1][:, 0, :, :]) - self.cov_func(head_covariances[1])
            query_covariance_2 = self.cov_func(query_covariance_2)
            if self.use_center_trans:
                query_center_2, query_covariance_2 = self.center_trans_network(query_center_2, query_covariance_2)
                query_covariance_2 = self.cov_func(query_covariance_2)

            query_center_2 = query_center_2 + relation_centers[2][:, 0, :, :]
            query_covariance_2 = self.cov_func(relation_covariances[2][:, 0, :, :]) - query_covariance_2
            query_covariance_2 = self.cov_func(query_covariance_2)
            if self.use_center_trans:
                query_center_2, query_covariance_2 = self.center_trans_network(query_center_2, query_covariance_2)
                query_covariance_2 = self.cov_func(query_covariance_2)
            ...
            score_ = new_query_center - tail_center
            offset_score_ = new_query_covariance - self.cov_func(tail_covariance)
            ...
            t_covariance = self.cov_func(head_covariance)
            for rel in range(rel_len):
                query_center = query_center + relation_center[:, rel, :, :]
                t_covariance = self.cov_func(relation_covariance[:, rel, :, :]) - t_covariance
                t_covariance = self.cov_func(t_covariance)
                if self.use_center_trans:
                    query_center, t_covariance = self.center_trans_network(query_center, t_covariance)
                    t_covariance = self.cov_func(t_covariance)
            ...
            if 'inter' not in qtype and 'union' not in qtype:
                ...
                offset_score_ = t_covariance - self.cov_func(tail_covariance)
                ...
                if 'inter' in qtype:
                    ...
                    offset_score_ = new_covariance.unsqueeze(1) - self.cov_func(tail_covariances[0])
                    ###################################################
                    if self.use_simple_trans:
                    ...
                    offset_score_ = new_query_covariance - self.cov_func(tail_covariances[0])
