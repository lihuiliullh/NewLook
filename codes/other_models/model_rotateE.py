...
class Query2box(nn.Module):
    ...
    def BoxTransE(self, head, relation, tail, mode, relation_offset, head_offset, tail_offset, rel_len, qtype):
        ...
        a = torch.chunk(a, rel_len, dim=0)
        b = torch.chunk(b, rel_len, dim=0)

        tails = torch.chunk(tail, rel_len, dim=0)
        tails_offset = torch.chunk(tail_offset, rel_len, dim=0)

        if rel_len == 2:
            new_query_center = self.center_sets(a[0].squeeze(1), b[0].squeeze(1),
                                                a[1].squeeze(1), b[1].squeeze(1))
            new_offset = self.offset_sets(a[0].squeeze(1), b[0].squeeze(1),
                                          a[1].squeeze(1), b[1].squeeze(1))

        elif rel_len == 3:
            new_query_center = self.center_sets(a[0].squeeze(1), b[0].squeeze(1),
                                                a[1].squeeze(1), b[1].squeeze(1),
                                                a[2].squeeze(1), b[2].squeeze(1))
            new_offset = self.offset_sets(a[0].squeeze(1), b[0].squeeze(1),
                                          a[1].squeeze(1), b[1].squeeze(1),
                                          a[2].squeeze(1), b[2].squeeze(1))
        ...
        offset_score_ = new_offset.unsqueeze(1) - tails_offset[0]
        ...
        score = self.gamma.item() - torch.norm(score_, p=2, dim=-1) - torch.norm(offset_score_, p=2, dim=-1)
        score_center = self.gamma2.item() - torch.norm(score_, p=2, dim=-1) - torch.norm(offset_score_, p=2, dim=-1)
        score_center_plus = self.gamma.item() - torch.norm(score_, p=2, dim=-1) - torch.norm(offset_score_, p=2, dim=-1)
