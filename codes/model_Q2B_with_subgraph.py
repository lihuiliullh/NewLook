...
class Query2box(nn.Module):
            ...
            for element in logical_triples:
                ...
                if operation == 'anchor':
                    ...
                    for n in all_anchors:
                        ...
                        result_map[n]['query_min'] = embed
                        result_map[n]['query_max'] = embed
                    ...
                    query_min = result_map[from_id]['query_min']
                    query_max = result_map[from_id]['query_max']
                    ...
                    query_min = query_min - 0.5 * self.func(offset_embed)
                    query_max = query_max + 0.5 * self.func(offset_embed)
                    ...
                    if len(data) == 2:
                        ...
                        aaa = result_map[head_id1]['query_min']
                        bbb = result_map[head_id1]['query_max']
                        query_min1 = aaa - 0.5 * self.func(offset_embed1)
                        query_max1 = bbb + 0.5 * self.func(offset_embed1)
                        offset1 = query_max1 - query_min1

                        center_embed2 = center_embed2 + rel_embed2
                        ccc = result_map[head_id2]['query_min']
                        ddd = result_map[head_id2]['query_max']
                        query_min2 = ccc - 0.5 * self.func(offset_embed2)
                        query_max2 = ddd + 0.5 * self.func(offset_embed2)
                        offset2 = query_max2 - query_min2

                        new_query_center = self.center_sets(center_embed1.squeeze(1), offset1.squeeze(1),
                                                            center_embed2.squeeze(1), offset2.squeeze(1)).unsqueeze(1)
                        new_offset = self.offset_sets(center_embed1.squeeze(1), offset1.squeeze(1),
                                                      center_embed2.squeeze(1), offset2.squeeze(1)).unsqueeze(1)
                        ...
                        # ----------
                        center_embed1 = center_embed1 + rel_embed1
                        query_min1 = result_map[head_id1]['query_min'] - 0.5 * self.func(offset_embed1)
                        query_max1 = result_map[head_id1]['query_max'] + 0.5 * self.func(offset_embed1)
                        offset1 = query_max1 - query_min1

                        center_embed2 = center_embed2 + rel_embed2
                        query_min2 = result_map[head_id2]['query_min'] - 0.5 * self.func(offset_embed2)
                        query_max2 = result_map[head_id2]['query_max'] + 0.5 * self.func(offset_embed2)
                        offset2 = query_max2 - query_min2

                        center_embed3 = center_embed3 + rel_embed3
                        query_min3 = result_map[head_id3]['query_min'] - 0.5 * self.func(offset_embed3)
                        query_max3 = result_map[head_id3]['query_max'] + 0.5 * self.func(offset_embed3)
                        offset3 = query_max3 - query_min3

                        new_query_center = self.center_sets(center_embed1.squeeze(1), offset1.squeeze(1),
                                                            center_embed2.squeeze(1), offset2.squeeze(1),
                                                            center_embed3.squeeze(1), offset3.squeeze(1)).unsqueeze(1)
                        new_offset = self.offset_sets(center_embed1.squeeze(1), offset1.squeeze(1),
                                                      center_embed2.squeeze(1), offset2.squeeze(1),
                                                      center_embed3.squeeze(1), offset3.squeeze(1)).unsqueeze(1)
                    ...
                    result_map[result_id] = {}
                    result_map[result_id]['center'] = center_embed1
                    result_map[result_id]['query_min'] = query_min1
                    result_map[result_id]['query_max'] = query_max1
                    ...
                    result_map[result_id] = {}
                    result_map[result_id]['center'] = [center_embed1, center_embed2]
    ...
    def BoxTransE(self, head, relation, tail, mode, offset, head_offset, rel_len, qtype):
        ...
        if qtype == 'chain-inter':
            ...
            if qtype == 'inter-chain':
                ...
                new_query_min = new_query_center - 0.5 * self.func(offsets[2][:, 0, :, :])
                new_query_max = new_query_center + 0.5 * self.func(offsets[2][:, 0, :, :])
            score_offset = F.relu(new_query_min - tail) + F.relu(tail - new_query_max)
            ...
            if 'inter' not in qtype and 'union' not in qtype and 'disjoin' not in qtype:
                ...
                if 'inter' in qtype:
                    ...
                    score_offset = score_center
                    score_center_plus = score_center
                elif 'union' in qtype:
    ...
    def check_accuracy(model, variable, mapping, res_mapping):
        ...
        res = Query2box.sort_score_get_k(scores, 50)
