...
from model_NewLook import NewLook
...
def main(args):
    ...
    if args.do_train:
        ...
        if '2d' in tasks:
            train_dataloader_2d_tail = DataLoader(
                TrainInterDataset(train_triples_2d, nentity, nrelation, args.negative_sample_size, train_ans,
                                  'tail-batch'),
                batch_size=args.batch_size,
                shuffle=True,
                num_workers=max(1, args.cpu_num),
    ...
    if args.init_checkpoint:
        ...
        newlook.load_state_dict(checkpoint['model_state_dict'])
        if args.do_train:
    ...
    def evaluate_test():
        ...
        average_d_metrics = collections.defaultdict(list)
        average_dx_metrics = collections.defaultdict(list)
        if '2i' in tasks:
            metrics = newlook.test_step(newlook, test_triples_2i, test_ans, test_ans_hard, args)
        ...
        if '3i' in tasks:
            metrics = newlook.test_step(newlook, test_triples_3i, test_ans, test_ans_hard, args)
            ...
            for metric in metrics:
                ...
                average_i_metrics[metric].append(metrics[metric])
        if '2c' in tasks:
            metrics = newlook.test_step(newlook, test_triples_2, test_ans, test_ans_hard, args)
        ...
        if '3c' in tasks:
            metrics = newlook.test_step(newlook, test_triples_3, test_ans, test_ans_hard, args)
            ...
            for metric in metrics:
                ...
                average_c2_metrics[metric].append(metrics[metric])
        if '1c' in tasks:
            metrics = newlook.test_step(newlook, test_triples, test_ans, test_ans_hard, args)
            ...
            for metric in metrics:
                ...
                average_c_metrics[metric].append(metrics[metric])
        if 'ci' in tasks:
            metrics = newlook.test_step(newlook, test_triples_ci, test_ans, test_ans_hard, args)
        ...
        if 'ic' in tasks:
            metrics = newlook.test_step(newlook, test_triples_ic, test_ans, test_ans_hard, args)
            ...
            for metric in metrics:
                ...
                average_ex_metrics[metric].append(metrics[metric])
        if '2u' in tasks:
            metrics = newlook.test_step(newlook, test_triples_2u, test_ans, test_ans_hard, args)
        ...
        if 'uc' in tasks:
            metrics = newlook.test_step(newlook, test_triples_uc, test_ans, test_ans_hard, args)
            ...
            for metric in metrics:
                ...
                average_u_metrics[metric].append(metrics[metric])
        if '2d' in tasks:
            metrics = newlook.test_step(newlook, test_triples_2d, test_ans, test_ans_hard, args)
            log_metrics('Test 2d', step, metrics)
            for metric in metrics:
                writer.add_scalar('Test_2d_'+metric, metrics[metric], step)
                average_metrics[metric].append(metrics[metric])
                average_d_metrics[metric].append(metrics[metric])
        if '3d' in tasks:
            metrics = newlook.test_step(newlook, test_triples_3d, test_ans, test_ans_hard, args)
            log_metrics('Test 3d', step, metrics)
            for metric in metrics:
                writer.add_scalar('Test_3d_'+metric, metrics[metric], step)
                average_metrics[metric].append(metrics[metric])
                average_d_metrics[metric].append(metrics[metric])
        if 'dc' in tasks:
            metrics = newlook.test_step(newlook, test_triples_dc, test_ans, test_ans_hard, args)
            log_metrics('Test dc', step, metrics)
            for metric in metrics:
                writer.add_scalar('Test_dc_'+metric, metrics[metric], step)
                average_metrics[metric].append(metrics[metric])
                average_dx_metrics[metric].append(metrics[metric])
        ...
        for metric in average_d_metrics:
            writer.add_scalar('Test_average_d_'+metric, np.mean(average_d_metrics[metric]), step)
        for metric in average_dx_metrics:
            writer.add_scalar('Test_average_dx_'+metric, np.mean(average_dx_metrics[metric]), step)
    ...
    def evaluate_val():
        ...
        if '2i' in tasks:
            metrics = newlook.test_step(newlook, valid_triples_2i, valid_ans, valid_ans_hard, args)
            ...
            for metric in metrics:
                ...
                average_i_metrics[metric].append(metrics[metric])
        if '3i' in tasks:
            metrics = newlook.test_step(newlook, valid_triples_3i, valid_ans, valid_ans_hard, args)
            ...
            for metric in metrics:
                ...
                average_i_metrics[metric].append(metrics[metric])
        if '2c' in tasks:
            metrics = newlook.test_step(newlook, valid_triples_2, valid_ans, valid_ans_hard, args)
            ...
            for metric in metrics:
                ...
                average_c2_metrics[metric].append(metrics[metric])
        if '3c' in tasks:
            metrics = newlook.test_step(newlook, valid_triples_3, valid_ans, valid_ans_hard, args)
            ...
            for metric in metrics:
                ...
                average_c2_metrics[metric].append(metrics[metric])
        if '1c' in tasks:
            metrics = newlook.test_step(newlook, valid_triples, valid_ans, valid_ans_hard, args)
            ...
            for metric in metrics:
                ...
                average_c_metrics[metric].append(metrics[metric])
        if 'ci' in tasks:
            metrics = newlook.test_step(newlook, valid_triples_ci, valid_ans, valid_ans_hard, args)
            ...
            for metric in metrics:
                ...
                average_ex_metrics[metric].append(metrics[metric])
        if 'ic' in tasks:
            metrics = newlook.test_step(newlook, valid_triples_ic, valid_ans, valid_ans_hard, args)
            ...
            for metric in metrics:
                ...
                average_ex_metrics[metric].append(metrics[metric])
        if '2u' in tasks:
            metrics = newlook.test_step(newlook, valid_triples_2u, valid_ans, valid_ans_hard, args)
            ...
            for metric in metrics:
                ...
                average_u_metrics[metric].append(metrics[metric])
        if 'uc' in tasks:
            metrics = newlook.test_step(newlook, valid_triples_uc, valid_ans, valid_ans_hard, args)
            ...
            for metric in metrics:
                ...
                average_u_metrics[metric].append(metrics[metric])
        if '2d' in tasks:
            metrics = newlook.test_step(newlook, valid_triples_2d, valid_ans, valid_ans_hard, args)
            log_metrics('Valid 2d', step, metrics)
            for metric in metrics:
                writer.add_scalar('Valid_2d_'+metric, metrics[metric], step)
                average_metrics[metric].append(metrics[metric])
                average_d_metrics[metric].append(metrics[metric])
        if '3d' in tasks:
            metrics = newlook.test_step(newlook, valid_triples_3d, valid_ans, valid_ans_hard, args)
            log_metrics('Valid 3d', step, metrics)
            for metric in metrics:
                writer.add_scalar('Valid_3d_'+metric, metrics[metric], step)
                average_metrics[metric].append(metrics[metric])
                average_d_metrics[metric].append(metrics[metric])
        if 'dc' in tasks:
            metrics = newlook.test_step(newlook, valid_triples_dc, valid_ans, valid_ans_hard, args)
            log_metrics('Valid dc', step, metrics)
            for metric in metrics:
                writer.add_scalar('Valid_dc_'+metric, metrics[metric], step)
                average_metrics[metric].append(metrics[metric])
                average_dx_metrics[metric].append(metrics[metric])
        ...
        for metric in average_d_metrics:
            writer.add_scalar('Valid_average_d_'+metric, np.mean(average_d_metrics[metric]), step)
        for metric in average_dx_metrics:
            writer.add_scalar('Valid_average_dx_'+metric, np.mean(average_dx_metrics[metric]), step)
    ...
    def evaluate_train():
        ...
        if '2i' in tasks:
            metrics = newlook.test_step(newlook, train_triples_2i, train_ans, train_ans, args)
            ...
            for metric in metrics:
                ...
                average_i_metrics[metric].append(metrics[metric])
        if '3i' in tasks:
            metrics = newlook.test_step(newlook, train_triples_3i, train_ans, train_ans, args)
            ...
            for metric in metrics:
                ...
                average_i_metrics[metric].append(metrics[metric])
        if '2c' in tasks:
            metrics = newlook.test_step(newlook, train_triples_2, train_ans, train_ans, args)
            ...
            for metric in metrics:
                ...
                average_c2_metrics[metric].append(metrics[metric])
        if '3c' in tasks:
            metrics = newlook.test_step(newlook, train_triples_3, train_ans, train_ans, args)
            ...
            for metric in metrics:
                ...
                average_c2_metrics[metric].append(metrics[metric])
        if '1c' in tasks:
            metrics = newlook.test_step(newlook, train_triples, train_ans, train_ans, args)
            ...
            for metric in metrics:
                ...
                average_c_metrics[metric].append(metrics[metric])
        if '2d' in tasks:
            metrics = newlook.test_step(newlook, train_triples_2d, train_ans, train_ans, args)
            log_metrics('train 2d', step, metrics)
            for metric in metrics:
                writer.add_scalar('train_2d_'+metric, metrics[metric], step)
                average_metrics[metric].append(metrics[metric])
                average_d_metrics[metric].append(metrics[metric])
        if '3d' in tasks:
            metrics = newlook.test_step(newlook, train_triples_3d, train_ans, train_ans, args)
            log_metrics('train 3d', step, metrics)
            for metric in metrics:
                writer.add_scalar('train_3d_'+metric, metrics[metric], step)
                average_metrics[metric].append(metrics[metric])
                average_d_metrics[metric].append(metrics[metric])
        ...
        for metric in average_d_metrics:
            writer.add_scalar('train_average_d_'+metric, np.mean(average_d_metrics[metric]), step)
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            ...
            if step >= begin_pq_step and not args.train_onehop_only:
                if '2i' in tasks:
                    log = newlook.train_step(newlook, optimizer, train_iterator_2i, args, step)
                    for metric in log:
                ...
                if '2d' in tasks:
                    log = newlook.train_step(newlook, optimizer, train_iterator_2d, args, step)
                    for metric in log:
                        writer.add_scalar('2d_' + metric, log[metric], step)
                    training_logs.append(log)

                if '3d' in tasks:
                    log = newlook.train_step(newlook, optimizer, train_iterator_3d, args, step)
                    for metric in log:
                        writer.add_scalar('3d_' + metric, log[metric], step)
                    training_logs.append(log)

            if '1c' in tasks:
                log = newlook.train_step(newlook, optimizer, train_iterator, args, step)
                for metric in log:
            ...
            if step % args.save_checkpoint_steps == 0:
                ...
                save_model(newlook, optimizer, save_variable_list, args)
    ...
    def evaluate_query():
        metrics = newlook.query_step(newlook, args.data_path)
