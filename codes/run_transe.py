...
# from model_box_neural import Query2box
from model_transe_with_subgraph import Query2box
...
def save_model(model, optimizer, save_variable_list, args, before_finetune=False):
    ...
    with open(os.path.join(args.save_path, 'config.json' if not before_finetune else 'config_before.json'),
              'w') as fjson:
...
def main(args):
    ...
    if (not args.do_train) and (not args.do_valid) and (not args.do_test) and (not args.evaluate_train) and (
    not args.do_query):
    ...
    if args.do_train:
        ...
        if '2i' in tasks:
            train_dataloader_2i_tail = DataLoader(
                TrainInterDataset(train_triples_2i, nentity, nrelation, args.negative_sample_size, train_ans,
                                  'tail-batch'),
                batch_size=args.batch_size,
                shuffle=True,
                num_workers=max(1, args.cpu_num),
        ...
        if '3i' in tasks:
            train_dataloader_3i_tail = DataLoader(
                TrainInterDataset(train_triples_3i, nentity, nrelation, args.negative_sample_size, train_ans,
                                  'tail-batch'),
                batch_size=args.batch_size,
                shuffle=True,
                num_workers=max(1, args.cpu_num),
    ...
    def evaluate_test():
        ...
        if 'uc' in tasks:
            ...
            for metric in metrics:
                ...
                average_u_metrics[metric].append(metrics[metric])
        if '2d' in tasks:
            metrics = query2box.test_step(query2box, test_triples_2d, test_ans, test_ans_hard, args)
        ...
        if '3d' in tasks:
            metrics = query2box.test_step(query2box, test_triples_3d, test_ans, test_ans_hard, args)
            ...
            for metric in metrics:
                ...
                average_d_metrics[metric].append(metrics[metric])
        if 'dc' in tasks:
            metrics = query2box.test_step(query2box, test_triples_dc, test_ans, test_ans_hard, args)
    ...
    def evaluate_val():
        ...
        if 'uc' in tasks:
            ...
            for metric in metrics:
                ...
                average_u_metrics[metric].append(metrics[metric])
        if '2d' in tasks:
            metrics = query2box.test_step(query2box, valid_triples_2d, valid_ans, valid_ans_hard, args)
            ...
            for metric in metrics:
                ...
                average_d_metrics[metric].append(metrics[metric])
        if '3d' in tasks:
            metrics = query2box.test_step(query2box, valid_triples_3d, valid_ans, valid_ans_hard, args)
            ...
            for metric in metrics:
                ...
                average_d_metrics[metric].append(metrics[metric])
        if 'dc' in tasks:
            metrics = query2box.test_step(query2box, valid_triples_dc, valid_ans, valid_ans_hard, args)
    ...
    def evaluate_train():
        ...
        if '1c' in tasks:
            ...
            for metric in metrics:
                ...
                average_c_metrics[metric].append(metrics[metric])
        if '2d' in tasks:
            metrics = query2box.test_step(query2box, train_triples_2d, train_ans, train_ans, args)
            ...
            for metric in metrics:
                ...
                average_d_metrics[metric].append(metrics[metric])
        if '3d' in tasks:
            metrics = query2box.test_step(query2box, train_triples_3d, train_ans, train_ans, args)
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            ...
            if step >= begin_pq_step and not args.train_onehop_only:
                ...
                if '2d' in tasks:
                    log = query2box.train_step(query2box, optimizer, train_iterator_2d, args, step)
                    for metric in log:
                        writer.add_scalar('2d_' + metric, log[metric], step)
                    training_logs.append(log)

                if '3d' in tasks:
                    log = query2box.train_step(query2box, optimizer, train_iterator_3d, args, step)
                    for metric in log:
    ...
    def evaluate_query():
        metrics = query2box.query_step(query2box)
