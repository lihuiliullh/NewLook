...
class Query2box(nn.Module):
    ...
    def BoxTransE(self, head, relation, tail, mode, offset, head_offset, rel_len, qtype):
        ...
        if qtype == 'chain-inter':
            ...
            query_center_1 = torch.matmul(query_center_1, relations[1][:,0,:,:])
            ...
            new_query_center = torch.matmul(conj_query_center, relations[2][:,0,:,:])
            ...
            query_center_1 = torch.matmul(heads[0], relations[0][:,0,:,:])
            query_center_1 = torch.matmul(query_center_1, relations[2][:,0,:,:])

            #query_center_2 = heads[1] + relations[1][:,0,:,:] + relations[2][:,0,:,:]
            query_center_2 = torch.matmul(heads[1], relations[1][:,0,:,:])
            query_center_2 = torch.matmul(query_center_2, relations[2][:,0,:,:])
            if self.euo:
            ...
            for rel in range(rel_len):
                ...
                aaa = relation[:,rel,:,:].squeeze(1)
                bbb = query_center
                query_center = torch.bmm(bbb, aaa)
            if self.euo:
