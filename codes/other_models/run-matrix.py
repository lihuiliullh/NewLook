...
#from model_gaussian import Query2box
from model_simple_matrix import Query2box
...
def main(args):
    ...
    with open('%s/initial_node_embedding.pkl' % args.data_path, 'rb') as handle:
        node_embedding_matrix = pickle.load(handle)

    with open('%s/initial_predicate_embedding.pkl' % args.data_path, 'rb') as handle:
        predicate_embedding_matrix = pickle.load(handle)
        for i in range(len(predicate_embedding_matrix)):
            for j in range(len(predicate_embedding_matrix[0])):
                for k in range(len(predicate_embedding_matrix[0][0])):
                    if predicate_embedding_matrix[i][j][k] > 1:
                        aaa = predicate_embedding_matrix[i][j][k]
                        predicate_embedding_matrix[i][j][k] = 1
    ...
    query2box = Query2box(
        ...
        initial_node_embedding = node_embedding_matrix,
        initial_predicate_embedding = predicate_embedding_matrix
    )
