...
class DisjoinNN(nn.Module):
    ...
    def __init__(self, center_dim, offset_dim):
        ...
        self.post_mats_center = nn.Parameter(torch.FloatTensor(expand_dim * 4, center_dim))
        # every time the initial is different
        nn.init.xavier_uniform(self.post_mats_center)
        self.register_parameter("post_mats_center", self.post_mats_center)

        self.post_mats_offset = nn.Parameter(torch.FloatTensor(expand_dim * 4, center_dim))
        # every time the initial is different
        nn.init.xavier_uniform(self.post_mats_offset)
        self.register_parameter("post_mats_offset", self.post_mats_offset)

        self.post_mats_offset2 = nn.Parameter(torch.FloatTensor(expand_dim * 4, center_dim))
        # every time the initial is different
        nn.init.xavier_uniform(self.post_mats_offset2)
        self.register_parameter("post_mats_offset2", self.post_mats_offset2)


    def forward(self, center_emb1, offset1, center_emb2, offset2):
        # if A and B are of shape (3, 4), torch.cat([A, B], dim=0) will be of shape (6, 4)
        # and torch.stack([A, B], dim=0) will be of shape (2, 3, 4).
        temp1 = F.relu(torch.matmul(center_emb1, self.mats1_center))
        temp11 = F.relu(torch.matmul(center_emb2, self.mats2_center))
        temp2 = F.relu(torch.matmul(offset1, self.mats1_offset))
        temp3 = F.relu(torch.matmul(offset2, self.mats1_offset2))

        temp4 = torch.cat([temp1, temp11, temp2, temp3], dim=2)
...
class CenterNetwork_AttentionSet(nn.Module):
    def __init__(self, mode_dims, expand_dims, center_use_offset, att_reg=0., att_tem=1., att_type="whole", bn='no',
        ...
        self.Attention_module = MLP(mode_dims, expand_dims, center_use_offset, att_type=att_type, bn=bn, nat=nat)
...
class Query2box(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma,
        ...
        self.normal_embedding = nn.Parameter(torch.zeros(nrelation, self.relation_dim, self.relation_dim))
        nn.init.uniform_(
            tensor=self.normal_embedding,
            a=-self.embedding_range.item(),
            b=self.embedding_range.item()
        )
        ...
        if self.geo == 'vec':
            if self.center_deepsets == 'vanilla':
                ...
                self.deepsets = CenterNetwork_AttentionSet(self.relation_dim, self.relation_dim, False,
    ...
    def forward(self, sample=None, rel_len=None, qtype=None, mode='single', find_subgraph=False, logical_triples=None, subgraph_info_map=None):
            ...
            for element in logical_triples:
                ...
                if operation == 'anchor':
                    ...
                    for n in all_anchors:
                        ...
                        embed = torch.index_select(self.entity_embedding, dim=0, index = torch.LongTensor([int(idx)]).cuda()).unsqueeze(1)
                        result_map[n] = {}
                    ...
                    rel_embed = torch.index_select(self.relation_embedding, dim=0, index = torch.LongTensor([int(predicate_id)]).cuda()).unsqueeze(1)
                    query_center = query_center + rel_embed
                    offset_embed = torch.index_select(self.offset_embedding, dim=0, index = torch.LongTensor([int(predicate_id)]).cuda()).unsqueeze(1)
                    ...
                    if len(data) == 2:
                        ...
                        rel_embed1 = torch.index_select(self.relation_embedding, dim=0, index = torch.LongTensor([int(rel_id1)]).cuda()).unsqueeze(1)
                        rel_embed2 = torch.index_select(self.relation_embedding, dim=0, index = torch.LongTensor([int(rel_id2)]).cuda()).unsqueeze(1)
                        offset_embed1 = torch.index_select(self.offset_embedding, dim=0, index = torch.LongTensor([int(rel_id1)]).cuda()).unsqueeze(1)
                        offset_embed2 = torch.index_select(self.offset_embedding, dim=0, index = torch.LongTensor([int(rel_id2)]).cuda()).unsqueeze(1)
                        # ----------
                        ...
                        new_query_center = self.center_sets(query_center1.squeeze(1), offset1.squeeze(1), query_center2.squeeze(1), offset2.squeeze(1)).unsqueeze(1)
                        new_offset = self.offset_sets(query_center1.squeeze(1), offset1.squeeze(1), query_center2.squeeze(1), offset2.squeeze(1)).unsqueeze(1)
                        ...
                        rel_embed1 = torch.index_select(self.relation_embedding, dim=0, index = torch.LongTensor([int(rel_id1)]).cuda()).unsqueeze(1)
                        rel_embed2 = torch.index_select(self.relation_embedding, dim=0, index = torch.LongTensor([int(rel_id2)]).cuda()).unsqueeze(1)
                        rel_embed3 = torch.index_select(self.relation_embedding, dim=0, index = torch.LongTensor([int(rel_id3)]).cuda()).unsqueeze(1)
                        offset_embed1 = torch.index_select(self.offset_embedding, dim=0, index = torch.LongTensor([int(rel_id1)]).cuda()).unsqueeze(1)
                        offset_embed2 = torch.index_select(self.offset_embedding, dim=0, index = torch.LongTensor([int(rel_id2)]).cuda()).unsqueeze(1)
                        offset_embed3 = torch.index_select(self.offset_embedding, dim=0, index = torch.LongTensor([int(rel_id3)]).cuda()).unsqueeze(1)
                        # ----------
                        center_embed1 = center_embed1 + rel_embed1
                        query_min1 = center_embed1 - 0.5 * self.func(offset_embed1)
                        query_max1 = center_embed1 + 0.5 * self.func(offset_embed1)
                        ...
                        new_query_center = self.center_sets(query_center1.squeeze(1), offset1.squeeze(1), query_center2.squeeze(1), offset2.squeeze(1), query_center3.squeeze(1), offset3.squeeze(1)).unsqueeze(1)
                        new_offset = self.offset_sets(query_center1.squeeze(1), offset1.squeeze(1), query_center2.squeeze(1), offset2.squeeze(1), query_center3.squeeze(1), offset3.squeeze(1)).unsqueeze(1)
                    ...
                    new_query_center, new_query_min, new_query_max = self.disjoin_nn(center_embed1, offset1,
                                                                                        center_embed2, offset2)
                    result_map[result_id] = {}
                    result_map[result_id]['center'] = new_query_center
                    result_map[result_id]['query_min'] = new_query_min
                    result_map[result_id]['query_max'] = new_query_max
        ...
        if qtype == 'chain-inter':
            ...
            if self.transH:
                relation_normal_11 = torch.index_select(self.normal_embedding, dim=0, index=head_part[:, 1]).unsqueeze(
                    1).unsqueeze(1)
                relation_normal_12 = torch.index_select(self.normal_embedding, dim=0, index=head_part[:, 2]).unsqueeze(
                    1).unsqueeze(1)
                relation_normal_2 = torch.index_select(self.normal_embedding, dim=0, index=head_part[:, 4]).unsqueeze(
                    1).unsqueeze(1)
                relation_normal = torch.cat([relation_normal_11, relation_normal_12, relation_normal_2], dim=0)
            ...
            if self.transH:
                relation_normal_11 = torch.index_select(self.normal_embedding, dim=0, index=head_part[:, 1]).unsqueeze(
                    1).unsqueeze(1)
                relation_normal_12 = torch.index_select(self.normal_embedding, dim=0, index=head_part[:, 3]).unsqueeze(
                    1).unsqueeze(1)
                relation_normal_13 = torch.index_select(self.normal_embedding, dim=0, index=head_part[:, 4]).unsqueeze(
                    1).unsqueeze(1)
                relation_normal = torch.cat([relation_normal_11, relation_normal_12, relation_normal_13], dim=0)
            ...
            if mode == 'single':
                ...
                if self.transH:
                    relation_normal_1 = torch.index_select(self.normal_embedding, dim=0, index=sample[:, 1]).unsqueeze(
                        1).unsqueeze(1)
                    relation_normal_2 = torch.index_select(self.normal_embedding, dim=0, index=sample[:, 3]).unsqueeze(
                        1).unsqueeze(1)
                    relation_normal = torch.cat([relation_normal_1, relation_normal_2], dim=0)
                    if rel_len == 3:
                        relation_normal_3 = torch.index_select(self.normal_embedding, dim=0, index=sample[:, 5]).unsqueeze(
                            1).unsqueeze(1)
                        relation_normal = torch.cat([relation_normal, relation_normal_3], dim=0)
                ...
                if self.transH:
                    relation_normal_1 = torch.index_select(self.normal_embedding, dim=0, index=head_part[:, 1]).unsqueeze(
                        1).unsqueeze(1)
                    relation_normal_2 = torch.index_select(self.normal_embedding, dim=0, index=head_part[:, 3]).unsqueeze(
                        1).unsqueeze(1)
                    relation_normal = torch.cat([relation_normal_1, relation_normal_2], dim=0)
                    if rel_len == 3:
                        relation_normal_3 = torch.index_select(self.normal_embedding, dim=0,
                                                        index=head_part[:, 5]).unsqueeze(1).unsqueeze(1)
                        relation_normal = torch.cat([relation_normal, relation_normal_3], dim=0)
            ...
            if mode == 'single':
                ...
                if self.transH:
                    relation_normal = torch.index_select(self.normal_embedding, dim=0, index=sample[:, 1]).unsqueeze(
                        1).unsqueeze(1)
                ...
                if rel_len == 2 or rel_len == 3:
                    ...
                    if self.transH:
                        relation_normal_2 = torch.index_select(self.normal_embedding, dim=0, index=sample[:, 2]).unsqueeze(
                            1).unsqueeze(1)
                        relation_normal = torch.cat([relation_normal, relation_normal_2], 1)
                ...
                if rel_len == 3:
                    ...
                    if self.transH:
                        relation_normal_3 = torch.index_select(self.normal_embedding, dim=0, index=sample[:, 3]).unsqueeze(
                            1).unsqueeze(1)
                        relation_normal = torch.cat([relation_normal, relation_normal_3], 1)
                ...
                if self.transH:
                    relation_normal = torch.index_select(self.normal_embedding, dim=0, index=head_part[:, 1]).unsqueeze(
                        1).unsqueeze(1)
                ...
                if rel_len == 2 or rel_len == 3:
                    ...
                    if self.transH:
                        relation_normal_2 = torch.index_select(self.normal_embedding, dim=0, index=head_part[:, 2]).unsqueeze(
                            1).unsqueeze(1)
                        relation_normal = torch.cat([relation_normal, relation_normal_2], 1)
                ...
                if rel_len == 3:
                    ...
                    if self.transH:
                        relation_normal_3 = torch.index_select(self.normal_embedding, dim=0, index=head_part[:, 3]).unsqueeze(
                            1).unsqueeze(1)
                        relation_normal = torch.cat([relation_normal, relation_normal_3], 1)
        ...
        if self.model_name in model_func:
            if qtype == '2-inter' or qtype == '3-inter' or qtype == '2-union' or qtype == '3-union' or qtype == '2-disjoin' or qtype == '3-disjoin':
                score, score_cen, offset_norm, score_cen_plus, _ = model_func[self.model_name](head, relation, tail,
                                                                                               mode, offset,
                                                                                               head_offset, 1, qtype, shift_of_node, relation_normal)
            else:
                score, score_cen, offset_norm, score_cen_plus, _ = model_func[self.model_name](head, relation, tail,
                                                                                               mode, offset,
                                                                                               head_offset, rel_len,
                                                                                               qtype, shift_of_node, relation_normal)
        else:
            raise ValueError('model %s not supported' % self.model_name)

        return score, score_cen, offset_norm, score_cen_plus, None, None

    def BoxTransE(self, head, relation, tail, mode, offset, head_offset, rel_len, qtype, shift_of_node, relation_normal = None):

        if qtype == 'chain-inter':
            # chain after intersection
            relations = torch.chunk(relation, 3, dim=0)
            if self.transH:
                relation_normals = torch.chunk(relation_normal, 3, dim=0)
            ...
            if qtype == 'inter-chain':
                ...
                disjoin_center, new_min, new_max = self.disjoin_nn(query_center_1, offset_1, query_center_2, offset_2)
            ...
            if 'inter' not in qtype and 'union' not in qtype and 'disjoin' not in qtype:
                ...
                if 'inter' in qtype:
                    ...
                    if rel_len == 2:
                        new_query_center, new_query_min, new_query_max = self.disjoin_nn(queries_center[0], offsets[0], queries_center[1], offsets[1])
                    elif rel_len == 3:
                        new_query_center = self.center_sets(queries_center[0].squeeze(1), offsets[0].squeeze(1),
                                                            queries_center[1].squeeze(1), offsets[1].squeeze(1))
                        new_offset = self.offset_sets(queries_center[0].squeeze(1), offsets[0].squeeze(1),
                        ...
                        new_query_center, new_query_min, new_query_max = self.disjoin_nn(new_query_center.unsqueeze(1), new_offset.unsqueeze(1),
                                                                             queries_center[2], offsets[2])
                    ...
                    score_offset = F.relu(new_query_min - tails[0] - true_shift) + F.relu(tails[0] + true_shift - new_query_max)
                    score_center = new_query_center - tails[0] - true_shift
                    # if tail inside the box, score_center_plus = tail
                    # otherwise, score_center_plus = new_query_max or new_query_min
                    # this is the inside box distance
                    score_center_plus = torch.min(new_query_max,
                                                  torch.max(new_query_min, tails[0] + true_shift)) - new_query_center
    ...
    @staticmethod
    def cal_distance(center_embed, box_min, box_max, candidates_embed):
    ...
    def check_accuracy(model, variable, mapping, res_mapping):
        ...
        scores = Query2box.cal_distance(res_center, res_min, res_max, all_embed)
    ...
    @staticmethod
    def cal_subgraph_acc(model, query_file):
        ...
        for e in subgraph_5_star:
            ...
            for v in variables:
                acc = Query2box.check_accuracy(model, v, mapping, res_map)
                if v not in result_acc_map:
                    result_acc_map[v] = []
                result_acc_map[v].append(acc)
        ...
        return result_acc_map

    @staticmethod
    def cal_subgraph_acc_union(model, query_file):
        with open(query_file, 'rb') as handle:
            subgraph_5_star = pickle.load(handle)
        ...
        for e in subgraph_5_star:
            ...
            for v in variables[0:-1]:
                ...
                result_acc_map[v].append(acc)
            v = variables[-1]
            acc = Query2box.check_accuracy_with_union(model, v, mapping, res_map)
            if v not in result_acc_map:
                result_acc_map[v] = []
            result_acc_map[v].append(acc)
        for key in result_acc_map.keys():
    ...
    def query_step(model):
        ...
        result_acc_map = Query2box.cal_subgraph_acc(model, query_file)
        print(result_acc_map)
        ...
        result_acc_map = Query2box.cal_subgraph_acc_union(model, query_file)
        print(result_acc_map)
