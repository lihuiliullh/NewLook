...
class Query2box(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma,
        ...
        self.entity_one_hot_embedding = nn.Parameter(torch.tensor(initial_node_embedding).float())
        self.relation_matrix_embedding = nn.Parameter(torch.tensor(initial_predicate_embedding).float())



        self.my_weight = nn.Parameter(torch.tensor([1]).float())
        self.my_weight2 = nn.Parameter(torch.tensor([1]).float())
    ...
    def forward(self, sample, rel_len, qtype, mode='single'):
        ...
        if qtype == 'chain-inter':
            ...
            if mode == 'single':
                ...
                one_hot_head_1 = torch.index_select(self.entity_one_hot_embedding, dim=0, index=sample[:, 0]).unsqueeze(1)
                one_hot_head_2 = torch.index_select(self.entity_one_hot_embedding, dim=0, index=sample[:, 2]).unsqueeze(1)
                ...
                ###############################################################################################
                relation_matrix_1 = torch.index_select(self.relation_matrix_embedding, dim=0, index=sample[:, 1]).unsqueeze(
                    1).unsqueeze(1)
                relation_matrix_2 = torch.index_select(self.relation_matrix_embedding, dim=0, index=sample[:, 3]).unsqueeze(
                    1).unsqueeze(1)
                relation_matrix = torch.cat([relation_matrix_1, relation_matrix_2], dim=0)
                if rel_len == 3:
                    relation_matrix_3 = torch.index_select(self.relation_matrix_embedding, dim=0, index=sample[:, 5]).unsqueeze(
                        1).unsqueeze(1)
                ...
                ###################################################
                one_hot_head_1 = torch.index_select(self.entity_one_hot_embedding, dim=0, index=head_part[:, 0]).unsqueeze(1)
                one_hot_head_2 = torch.index_select(self.entity_one_hot_embedding, dim=0, index=head_part[:, 2]).unsqueeze(1)
                one_hot_head = torch.cat([one_hot_head_1, one_hot_head_2], dim=0)
                if rel_len == 3:
                    one_hot_head_3 = torch.index_select(self.entity_one_hot_embedding, dim=0, index=head_part[:, 4]).unsqueeze(1)
                    one_hot_head = torch.cat([one_hot_head, one_hot_head_3], dim=0)
                ###################################################
                ...
                ########################################################
                relation_matrix_1 = torch.index_select(self.relation_matrix_embedding, dim=0, index=head_part[:, 1]).unsqueeze(
                    1).unsqueeze(1)
                relation_matrix_2 = torch.index_select(self.relation_matrix_embedding, dim=0, index=head_part[:, 3]).unsqueeze(
                    1).unsqueeze(1)
                relation_matrix = torch.cat([relation_matrix_1, relation_matrix_2], dim=0)
                if rel_len == 3:
                    relation_matrix_3 = torch.index_select(self.relation_matrix_embedding, dim=0, index=head_part[:, 5]).unsqueeze(
                        1).unsqueeze(1)
                    relation_matrix = torch.cat([relation_matrix, relation_matrix_3], dim=0)
                #########################################################
        ...
        if self.model_name in model_func:
            if qtype == '2-inter' or qtype == '3-inter' or qtype == '2-union' or qtype == '3-union':
                score, score_cen, offset_norm, score_cen_plus, _ = model_func[self.model_name](head, relation, tail,
                                                                                               ...
                                                                                               1, qtype, one_hot_head, relation_matrix, tail_one_hot)
            else:
                score, score_cen, offset_norm, score_cen_plus, _ = model_func[self.model_name](head, relation, tail,
                                                                                               mode, relation_offset,
                                                                                               head_offset, tail_offset,
                                                                                               rel_len, qtype, one_hot_head, relation_matrix, tail_one_hot)
        else:
    ...
    def BoxTransE(self, head, relation, tail, mode, relation_offset, head_offset, tail_offset, rel_len, qtype, head_one_hot, relation_matrix, tail_one_hot):
        ...
        score = self.gamma.item() - torch.norm(score_, p=1, dim=-1) - torch.norm(offset_score_, p=1, dim=-1) * self.my_weight - dist * self.my_weight2
        score_center = self.gamma2.item() - torch.norm(score_, p=1, dim=-1) - torch.norm(offset_score_, p=1, dim=-1) * self.my_weight - dist * self.my_weight2
        score_center_plus = self.gamma.item() - torch.norm(score_, p=1, dim=-1) - torch.norm(offset_score_, p=1, dim=-1) * self.my_weight - dist * self.my_weight2
