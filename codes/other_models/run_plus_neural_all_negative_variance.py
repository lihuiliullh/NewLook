...
from model_plus_neural_negative_variance import Query2box
...
def parse_args(args=None):
    ...
    parser.add_argument('--evaluate_train', action='store_false', help='Evaluate on training data')
...
def save_model(model, optimizer, save_variable_list, args, before_finetune=False):
    ...
    entity_center_embedding = model.entity_center_embedding.detach().cpu().numpy()
    np.save(
        os.path.join(args.save_path, 'entity_center_embedding' if not before_finetune else 'entity_embedding_before'),
        entity_center_embedding
    )

    relation_center_embedding = model.relation_center_embedding.detach().cpu().numpy()
    np.save(
        os.path.join(args.save_path, 'relation_center_embedding' if not before_finetune else 'relation_embedding_before'),
        relation_center_embedding
    )

    entity_covariance_embedding = model.entity_covariance.detach().cpu().numpy()
    np.save(
        os.path.join(args.save_path, 'entity_covariance' if not before_finetune else 'entity_covariance_before'),
        entity_covariance_embedding
    )

    relation_covariance = model.relation_covariance.detach().cpu().numpy()
    np.save(
        os.path.join(args.save_path, 'relation_covariance' if not before_finetune else 'relation_covariance_before'),
        relation_covariance
    )
...
def main(args):
    ...
    query2box = Query2box(
        ...
        use_simple_trans = False,
        use_center_trans = False
    )
