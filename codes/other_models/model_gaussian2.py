...
class Query2box(nn.Module):
    ...
    def BoxTransE(self, head, relation, tail, mode, offset, head_offset, rel_len, qtype):
        ...
        if qtype == 'chain-inter':
            ...
            variancecs = None
            if self.euo:
                variancecs = self.func(head_offset)
            else:
                variancecs = self.func(query_center)

            for rel in range(0, rel_len):
                variancecs = self.func(offset[:,rel,:,:] - variancecs)
            ...
            if 'inter' not in qtype and 'union' not in qtype:
                ...
                offsets = torch.chunk(variancecs, rel_len, dim=0)
                if 'inter' in qtype:
                    if rel_len == 2:
