...
class TransEDisjoinNN(nn.Module):
    # torch min will just return a single element in the tensor
    def __init__(self, center_dim, offset_dim):
        super(TransEDisjoinNN, self).__init__()
        self.center_dim = center_dim
        self.offset_dim = offset_dim
    ...
    def forward(self, center_emb1, center_emb2):
        # if A and B are of shape (3, 4), torch.cat([A, B], dim=0) will be of shape (6, 4)
        # and torch.stack([A, B], dim=0) will be of shape (2, 3, 4).
        ...
        if False:
            ...
            center_emb1 = center_emb1.unsqueeze(1)
            center_emb2 = center_emb2.unsqueeze(1)
            temp1 = torch.matmul(center_emb1, self.attention1)
            temp2 = torch.matmul(center_emb2, self.attention2)
            combined1 = F.softmax(torch.cat([temp1, temp2], dim=1), dim=1)
            q1 = (combined1[:, 0].view(center_emb1.size(0), 1)).unsqueeze(1)
...
class Query2box(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma, 
        ...
        self.transE_disjoin_nn = TransEDisjoinNN(hidden_dim, hidden_dim)
            ...
            for element in logical_triples:
                ...
                if operation == 'anchor':
                    ...
                    for n in all_anchors:
                        ...
                        result_map[n]['query_min'] = 0
                        result_map[n]['query_max'] = 0
                    ...
                    result_map[result_id]['query_min'] = 0
                    result_map[result_id]['query_max'] = 0
                    ...
                    if len(data) == 2:
                        ...
                        new_query_center = self.deepsets(center_embed1.squeeze(1), None, center_embed2.squeeze(1), None)
                        ...
                        new_query_center = self.deepsets(center_embed1.squeeze(1), None, center_embed2.squeeze(1), None, center_embed3.squeeze(1), None)
                    ...
                    center_embed1 = center_embed1 + rel_embed1
                    center_embed2 = center_embed2 + rel_embed2
                    center_emb = self.transE_disjoin_nn(center_embed1, center_embed2)

                    result_map[result_id] = {}
                    result_map[result_id]['center'] = center_emb
                    result_map[result_id]['query_min'] = 0
                    result_map[result_id]['query_max'] = 0
    ...
    def TransE(self, head, relation, tail, mode, offset, head_offset, rel_len, qtype):
        ...
        if qtype == 'chain-inter':
            ...
            if qtype == 'disjoin-chain':
                score_1 = (heads[0] + relations[0][:, 0, :, :])
                score_2 = (heads[1] + relations[1][:, 0, :, :])
                disjoin_center = self.transE_disjoin_nn(score_1, score_2)
                score = disjoin_center + relations[2][:, 0, :, :] - tail
            ...
            if 'inter' not in qtype and 'union' not in qtype and 'disjoin' not in qtype:
                ...
                if 'inter' in qtype:
                    ...
                    if rel_len == 2:
                        conj_score = self.transE_disjoin_nn(scores[0], scores[1])
                    elif rel_len == 3:
                        conj_score = self.deepsets(scores[0], None, scores[1], None)
                        conj_score = self.transE_disjoin_nn(conj_score, scores[2])
                    conj_score = conj_score.unsqueeze(1)
                    score = conj_score - tails[0]
                else:
    ...
    @staticmethod
    def cal_distance(center_embed, candidates_embed):
        # distance = center_embed - candidates_embed
        # score = -torch.norm(distance, p=1, dim=-1)
        score = center_embed - candidates_embed
    ...
    def check_accuracy(model, variable, mapping, res_mapping):
        ...
        scores = Query2box.cal_distance(res_center, all_embed)
    ...
    def check_accuracy_with_union(model, union_variable, mapping, res_mapping):
        ...
        scores = Query2box.cal_distance(new_query_center, all_embed)
